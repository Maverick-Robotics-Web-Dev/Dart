...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  Container _bodyPage(BuildContext context, SignUpState state) {
    return Container(
      ...
      child: SingleChildScrollView(
        child: Column(
          children: [
            _iconLabel(),
            _txtLabel(),
            _txtFormFieldName(state),
            _txtFormFieldLastName(state),
            _txtFormFieldEmail(state),
            _txtFormFieldPhone(state),
            _txtFormFieldPassword(state),
            _txtFormFieldConfirmPassword(state),
            _btnSignUp(context, state),
          ],
        ),
      ),
    );
  ...
  Icon _iconLabel() {
    return Icon(Icons.person, color: Colors.white, size: 100);
  }
  ...
  Widget _btnSignUp(BuildContext context, SignUpState state) {
    return Container(
      width: MediaQuery.of(context).size.width,
      height: 50,
  ...
  Widget _txtFormFieldConfirmPassword(SignUpState state) {
    return Padding(
  ...
  Widget _txtFormFieldPassword(SignUpState state) {
    return Padding(
      padding: EdgeInsets.only(top: 12, right: 24, left: 24),
      child: TxtFormFieldCustom(
        label: 'Contraseña',
  ...
  Widget _txtFormFieldPhone(SignUpState state) {
    return Padding(
      padding: EdgeInsets.only(top: 12, right: 24, left: 24),
      child: TxtFormFieldCustom(
        label: 'Telefono',
        ...
        onChanged: (text) {
          _signUpBloc?.add(PhoneChangedEvent(phone: BlocFormItem(value: text)));
        },
      ),
    );
  }

  Widget _txtFormFieldEmail(SignUpState state) {
    return Padding(
      padding: EdgeInsets.only(top: 12, right: 24, left: 24),
      child: TxtFormFieldCustom(
        label: 'E-mail',
        ...
        onChanged: (text) {
          _signUpBloc?.add(EmailChangedEvent(email: BlocFormItem(value: text)));
        },
      ),
    );
  }

  Widget _txtFormFieldLastName(SignUpState state) {
    return Padding(
      padding: EdgeInsets.only(top: 12, right: 24, left: 24),
      child: TxtFormFieldCustom(
        label: 'Apellido',
  ...
  Widget _txtFormFieldName(SignUpState state) {
    return Padding(
